...
async function updateOrder(order) {
  return fetch(
    " http://df54-2405-9800-b840-ee03-84dc-3a08-d5a0-96ce.ngrok.io/api/payment/upload",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        ...
        userId: order.userId,
        paymentSlip: order.postImage.myFile,
      }),
    }
  );
  // .then(data => data.json())
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await axios(
        " http://df54-2405-9800-b840-ee03-84dc-3a08-d5a0-96ce.ngrok.io/api/orderDetail/orderdetail/" +
          order.orderId
      );

      const result2 = await axios(
        " http://df54-2405-9800-b840-ee03-84dc-3a08-d5a0-96ce.ngrok.io/api/order/getSumPrice/" +
          order.orderId
      );

      const result3 = await axios(
        " http://df54-2405-9800-b840-ee03-84dc-3a08-d5a0-96ce.ngrok.io/api/address/orderAddress/" +
          order.addressId
      );
      ...
      >
        {hid === "All" ? (
          order.status === "Cancelled" ? (
            <a href="#" className="cat-menu">
              <h4 className="orderDetail" style={{ color: "red" }}>
                {" "}
        ...
        {hid === "All" ? (
          order.status === "Waiting for payment approval" ? (
            <a href="#" className="cat-menu">
              <h4 className="orderDetail" style={{ color: "blue" }}>
                {" "}
                {order.orderId} - {status}{" "}
              </h4>{" "}
            </a>
        ...
        {hid === "All" ? (
          order.status === "Completed" ? (
            <a href="#" className="cat-menu">
              <h4 className="orderDetail" style={{ color: "green" }}>
                {" "}
                {order.orderId} - {status}{" "}
              </h4>{" "}
            </a>
        ...
        {hid === "All" ? (
          order.status === "Confirm , Waiting for shipment" ? (
            <a href="#" className="cat-menu">
              <h4 className="orderDetail" style={{ color: "orange" }}>
                {" "}
                {order.orderId} - {status}{" "}
              </h4>{" "}
            </a>
        ...
        {hid === "All" ? (
          order.status === "Shipping" ? (
            <a href="#" className="cat-menu">
              <h4 className="orderDetail" style={{ color: "black" }}>
                {" "}
                {order.orderId} - {status} - {order.trackingNo}{" "}
              </h4>{" "}
            </a>
          ) : null
        ) : null}
        ...
        {hid === "All" ? (
          order.status === "Waiting for payment" ? (
            <a href="#" className="cat-menu">
              <h4 className="orderDetail" style={{ color: "purple" }}>
                {" "}
                {order.orderId} - {status}{" "}
              </h4>{" "}
            </a>
        ...
        {hid === "In Process Order" ? (
          order.status === "Shipping" ? (
            <a href="#" className="cat-menu">
              <h4 className="orderDetail" style={{ color: "violet" }}>
                {" "}
                {order.orderId} - {status} -{" "}
                <a className="trackingNo" onClick={handleClickTracking}>
                  {order.trackingNo}
                </a>{" "}
              </h4>{" "}
            </a>
          ...
          <MediaQuery minWidth={1224}>
            <Row>
              ...
              <Col>
                ...
                <img
                  style={{ width: "100%" }}
                  src="https://www.img.in.th/images/69ea02dea737ab4cba39501f8d9ba2bf.jpg"
                />
                ...
                {isUpload ? (
                  <h5 style={{ fontSize: "14px", color: "green" }}>
                    {" "}
                    You already upload your slip. Please waiting for
                    confirmation.
                  </h5>
                ) : null}

                {order.status === "Waiting for payment approval" ? (
                  <h5 style={{ fontSize: "16px", color: "blue" }}>
                    {" "}
                    You already upload your slip. Please waiting for
                    confirmation.
                  </h5>
                ...
                {order.status === "Confirm , Waiting for shipment" ? (
                  <h5 style={{ fontSize: "14px", color: "green" }}>
                    {" "}
                    Order has been confirmed. Waiting for shipping.
                  </h5>
                ) : null}
          ...
          <MediaQuery maxWidth={1224}>
            ...
            <Row>
              <Col>
                ...
                <h5 style={{ fontSize: "16px" }}>
                  {" "}
                  SCB - 28232512845 - Mymom Bakery Store{" "}
                </h5>
                ...
                {order.status === "Cancelled" ? (
                  <h5 style={{ fontSize: "14px", color: "red" }}>
                    {" "}
                    Your order has been canceled.
                  </h5>
                ) : (
                ...
                {order.status === "Waiting for payment approval" ? (
                  <h5 style={{ fontSize: "14px", color: "blue" }}>
                    {" "}
                    You already upload your slip. Please waiting for
                    confirmation.
                  </h5>
